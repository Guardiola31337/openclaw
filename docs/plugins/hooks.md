...
- Use `kind: "decision"` plus `decision` when the action resolves the approval
  ...
  buttons. Use `kind: "command"` for plugin-owned commands that collect more
  context or verify an external workflow before resolving the approval; native
  clients should leave those as visible command text.
